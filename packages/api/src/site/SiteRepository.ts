...
export const makeSiteRepository = (db: Db): SiteRepository => ({
  list: (ownerId) =>
    db
      .prepare(
        "SELECT document FROM sites WHERE owner_id = ? ORDER BY updated_at DESC",
      )
      .bind(ownerId)
      .all<{ document: string }>()
      .pipe(
      ...
      .pipe(
        Effect.flatMap((result) =>
          result.meta.changes === 0
            ? Effect.fail(new SiteNotFound({ id }))
            : Effect.void,
        ),
      ),
    ...
    Effect.gen(function* () {
      ...
      if (!verified) {
        return yield* Effect.fail(
          new DomainNotVerified({ domain: pending.domain }),
        )
      }
      ...
      const twin = pending.domain.startsWith("www.")
        ...
        : `www.${pending.domain}`
      yield* db
        .prepare("UPDATE site_domains SET verified = 1 WHERE site_id = ?")
        .bind(id)
        .run()
      yield* db
        .prepare(
