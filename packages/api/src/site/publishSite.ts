...
export const publishSite = Effect.fn("api.publishSite")(function* (siteId: string) {
  ...
  const site = yield* repo.get(siteId, requester);
  ...
  yield* repo.markPublished(siteId, requester, publishedAt);
  const sink = yield* BuildJobSink;
  yield* sink
    .send({
      siteId: site.id,
      // The build worker records the outcome scoped to the managing owner —
      // never the publishing requester (a client with `canPublish`).
