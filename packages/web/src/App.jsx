  ...
  return (
    <div className="auth-screen">
      <div className="auth-card">
        <div className="brand"><span className="brand-mark">✳</span><span>site<span className="brand-dot">.</span>studio</span></div>
        <h1>Welcome back</h1>
...
function Editor({ site, online, saveState, onUpdate, onPublish, onSetDomain, onVerifyDomain, onRemoveDomain }) {
    ...
    <div className="editor-layout">
      ...
      <div className="preview-area">
        <div className="preview-toolbar">
          <div><span className="live-dot" /> Live preview <span className="toolbar-divider" /> <span className="muted">Changes save automatically</span></div>
...
export default function App() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setOnline(false)
      setBanner({ kind: 'error', message: 'API unreachable — showing local demo data. Start the API and reload to go live.' })
    }
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      ...
      setBanner({ kind: 'success', message: 'Published. Static HTML is generated by the Astro build step (run `bun run publish` in packages/site-template).' })
    } catch (e) {
  ...
  const handleSetDomain = useCallback(async (domain) => {
    const id = await ensurePersisted()
