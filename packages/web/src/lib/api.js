...
async function request(path, options = {}) {
  ...
  if (!res.ok) {
    ...
    const err = new Error(detail?._tag ?? detail?.error ?? `Request failed (${res.status})`);
...
export const api = {
  ...
  updateSite: (id, site) => request(`/sites/${id}`, { method: "PUT", body: JSON.stringify(site) }),
  ...
  deleteLead: (siteId, leadId) => request(`/sites/${siteId}/leads/${leadId}`, { method: "DELETE" }),
};
